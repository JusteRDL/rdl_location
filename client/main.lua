...
local function createMenuPanes()
    RMenu.Add(cat, sub("main"), RageUI.CreateMenu(title, desc, nil, nil, "rdldev", "black"))
    ...
    RMenu.Add(cat, sub("confirm"), RageUI.CreateMenu(title, desc, nil, nil, "rdldev", "black"))
            ...
            RageUI.IsVisible(RMenu:Get(cat, sub("confirm")), true, true, true, function()
                ...
                RageUI.ButtonWithStyle("Louer ce véhicule", "Appuyez pour louer ce véhicule", {RightLabel = "~g~"..customGroupDigits(Config.location[selectedVehicle].price).."$~s~ →→"}, not serverInteraction, function(_,_,s)
                    if s then
                        ...
                        TriggerServerEvent("rdl:rent", selectedVehicle)
                    end
                end)
            end, function()
            end)
...
RegisterNetEvent("rdl:cb")
AddEventHandler("rdl:cb", function(sucess, message, vehicleData)
